...
def GetAccelerometerPosition(client, accel_last, init_time, file):
    ...
    if accel_squared>0.1 and flag_acc:
        message = {"accel": {"value": 0}}
        client.publish(topic="/v1.6/devices/test" , payload=json.dumps(message), qos=1, retain=False ) 
        #para escrever no ficheiro
        accel_squared=0
        flag_acc=False        
    elif accel_squared>0.1:
        if not flag_acc:
            message = {"accel": {"value": 0}}
            client.publish(topic="/v1.6/devices/test" , payload=json.dumps(message), qos=1, retain=False )
            flag_acc = True

        message = {"accel": {"value": form(accel_squared)} }
        client.publish(topic="/v1.6/devices/test" , payload=json.dumps(message), qos=1, retain=False )
...
if __name__ == '__main__':
    ...
    while True:
        ...
        temp_media = temp_media + body_temp



        data.append(body_temp)
